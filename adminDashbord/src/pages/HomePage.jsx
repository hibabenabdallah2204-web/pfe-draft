...
function getAssetIcon(assetType) {
  if (assetType?.includes("Building") || assetType?.includes("Immeuble")) return <Building2 size={16} />;
  if (assetType?.includes("House") || assetType?.includes("Maison")) return <Home size={16} />;
  if (assetType?.includes("Bare Land") || assetType?.includes("nu")) return <LandPlot size={16} />;
  if (assetType?.includes("Agricultural") || assetType?.includes("agricole")) return <Sprout size={16} />;
  ...
  return (
    <div className="group overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-xl shadow-slate-200/40 transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-500/10">
      <div className="relative h-56 w-full bg-slate-100 flex flex-col items-center justify-center overflow-hidden">
        {/* Placeholder gradient background to simulate premium images */}
      ...
      <div className="p-6 relative">
        <div className="absolute -top-6 right-6 rounded-2xl bg-white p-1.5 shadow-md">
        ...
        <p className="mt-2 flex items-center gap-2 text-sm text-slate-500">
          ...
          {asset.location || asset.t("home.confidential")}
        </p>

        <div className="mt-6 flex items-center gap-4">
           {asset.targetAmount && (
              <div className="flex-1 rounded-2xl bg-slate-50 p-3">
                 <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{asset.t("home.targetRaise")}</p>
                 <p className="mt-1 text-lg font-black text-fuchsia-600">{asset.targetAmount.toLocaleString()} TND</p>
              </div>
           )}
        </div>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-slate-100 pt-4">
          <p className="text-xs font-medium text-slate-400">
            {asset.status === "FUNDING_OPEN" ? asset.t("home.fundingActive") : asset.t("home.published")}
          </p>
          <button 
            onClick={() => window.location.href = `/project/${asset.id}`}
            className="flex items-center gap-1.5 rounded-2xl bg-gradient-to-r from-indigo-500 to-fuchsia-500 px-4 py-2.5 text-sm font-bold text-white shadow-md transition hover:shadow-lg focus:ring-4 focus:ring-fuchsia-500/20"
          >
            {asset.t("home.invest")} <ArrowRight size={16} />
          </button>
        </div>
      </div>
    </div>
...
export default function HomePage() {
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter((asset) => {
      ...
      // Category matching updated for new nomenclature
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-2xl bg-white border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 hover:text-rose-600 focus:ring-4 focus:ring-slate-100"
            >
              <LogOut size={16} />
              {t("nav.signOut")}
            </button>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8 md:py-16">
        ...
        <div className="mt-12 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full max-w-md">
            <Search
              className="absolute rtl:right-5 rtl:left-auto left-5 top-1/2 -translate-y-1/2 text-slate-400"
              size={20}
            />
            <input
              type="text"
              ...
              className="h-14 w-full rounded-3xl border border-slate-200 bg-white rtl:pr-12 rtl:pl-4 pl-12 pr-4 text-[15px] font-medium text-slate-700 outline-none shadow-sm transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-50"
            />
          </div>
